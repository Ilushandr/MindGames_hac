...
API_URL = 'https://go-backend-denis.ambersoft.llc'
...
@app.route('/')
def index():
    return render_template('index.html', lobby_id=None, lobbies=[])
...
def login():
    ...
    if form.validate_on_submit():
        token = post(API_URL + '/user/login',
                     {'email': form.email.data, 'password': form.password.data}).json()['token']
        session['token'] = token
...
def register():
    form = RegisterForm()
    if form.validate_on_submit():
        token = post(API_URL + '/user/register',
                     {'email': form.email.data, 'nickname': form.nickname.data}).json()['token']
        session['token'] = token
...
def create_lobby():
    post(API_URL + '/game/create/code', {'token': session['token']})
    redirect
